...
namespace Yngin {
	...
	uint32_t TexturesManager::createTexture(int width, int height, int numCh, const char* data) {
		auto texture = std::unique_ptr<Texture>(new Texture(impl->ctx, width, height, numCh, data));
